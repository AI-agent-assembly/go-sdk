...
go-sdk's reader-facing Markdown. AAASM-5798 / AAASM-5599.

Adapted from agent-assembly's `scripts/check_claim_vocabulary.py` (AAASM-5679),
via docs Hub's (AAASM-5797), python-sdk's and node-sdk's (AAASM-5798) ports.
Per AAASM-5795's federation-by-convention decision this is a ONE-TIME PORT,
not a shared dependency -- this file diverges independently from here. This
repo is Go, so the checker itself stays Python and CI installs Python 3.12
to run it -- AAASM-5795 already concluded Node/Go repos would shell out to a
Python process either way.

File scope: `docs/**` (Hugo content) and `README.md`/`CONTRIBUTING.md`.
Excludes `metadata/quickstart/**` -- vendored, verbatim copies from the
`examples` repo (its own README states "Vendored, not fetched"; same
reasoning as python-sdk's `quickstart_snippets/` exclusion) -- and
...
EXCLUDE_GLOBS = (
    # Vendored, verbatim copies from the `examples` repo -- its own README
    # states "Vendored, not fetched"; same reasoning as python-sdk's
    # quickstart_snippets/ exclusion.
    "metadata/quickstart/**",
...
def _opens_front_matter(lines: list[str]) -> bool:
    ...
    break. Hugo pages carry real front matter (docs/TEMPLATE.md's `title:`/
    `weight:`/`sidebar:` block, for one), so this check is real."""
...
def selftest() -> int:
    ...
    if not in_scope("docs/quick-start.md"):
        failures.append("in_scope() rejects a real docs/ page")
    if in_scope("metadata/quickstart/_snippets/langchaingo.go.txt"):
        failures.append("in_scope() accepts a vendored quickstart snippet")
    if in_scope("verification-reports/AAASM-2704.md"):
        failures.append("in_scope() accepts a verification report")
    if in_scope("docs/quick-start.txt"):
